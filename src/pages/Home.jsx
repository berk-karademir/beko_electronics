...
const Home = () => {
  ...
  const currentHeading =
    SORT_OPTIONS.find((opt) => opt.value === sort)?.heading || "Ürünler";
  ...
  return (
    <main className="container mx-auto py-8 px-4">
      ...
      <div className="flex gap-22">
        ...
        <TopRatedProductsCarousel products={topRatedProducts} />
      </div>
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {paginatedProducts.map((product) => (
            ...
            >
              ...
              <div className="font-semibold mb-1 text-center mt-5">
                {product.title}
              </div>
              <div className="text-blue-700 font-bold mb-1">
                ${product.price}
              </div>
              <div className="text-sm text-gray-500 text-center line-clamp-2">
                {product.description}
              </div>
              <button 
                onClick={(e) => {
              ...
              <div className="flex gap-2 mt-2 text-xs text-gray-500 items-center">
                ...
                <span className="flex items-center gap-1">
                  <MessageCircle size={14} /> {product.reviews?.length || 0}{" "}
                  reviews
                </span>
              </div>
            </Link>
          ...
          {[...Array(totalPages).keys()].map((n) => (
            <button
              ...
              className={`px-3 py-1 rounded border ${
                currentPage === n + 1 ? "bg-blue-700 text-white" : "bg-white"
              }`}
            >
              {n + 1}
            </button>
